...
class LeRobotUnitreeG1DataConfig(DataConfigFactory):
    """Data config for fine-tuning on a Unitree G1 (Dex1 bimanual) LeRobot v2.1 dataset.

    The G1 dataset stores a 16-dim absolute-joint-position state/action vector and three cameras
    (cam_left_high, cam_left_wrist, cam_right_wrist). Actions are *absolute* joint positions, so we do
    NOT apply a delta transform (see g1_policy.py for the joint layout).
    """

    # Injected as the prompt for every sample (the dataset's tasks.jsonl is a placeholder).
    # Must be character-identical to the prompt used at deployment time.
    default_prompt: str | None = None

    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Remap raw LeRobot column names to the flat keys expected by G1Inputs (and by the inference client).
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "observation/cam_left_high": "observation.images.cam_left_high",
        ...
        # NOTE: actions are absolute joint positions; no DeltaActions transform (deltas explode under
        # normalization because actions ~= current state).
...
_CONFIGS = [
    TrainConfig(
      ...
      data=LeRobotUnitreeG1DataConfig(
        repo_id="XiaoweiLinXL/unitree_insert_the_ethernet_cable_to_the_tv_box",
    ...
    TrainConfig(
      ...
      data=LeRobotGo2SuctionDataConfig(
        repo_id="local/go2_suction_extract_replay_v1_head3",
    ...
    #
    # Fine-tuning Unitree G1 configs.
    #
    # Fine-tunes pi0.5 on a Unitree G1 (Dex1 bimanual) LeRobot v2.1 dataset.
    # See g1_policy.py / LeRobotUnitreeG1DataConfig for the data mapping. Norm stats use a min/max
    # override (see scripts comment / norm_stats.json) so normalized actions stay bounded to ~[-1, 1];
    # plain quantile norm leaves rare wrist-joint extensions at absmax ~4 and degrades deployment.
    TrainConfig(
